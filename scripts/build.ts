...
function buildSeason(season: number): void {
  ...
  const howBlock = (teamNote: string) =>
    ...
    `<p><b>Outline is how easily he fits, and it comes from his diagrams.</b> Squares and rectangles sit flush; circles, diamonds and stars leave white around them. The five Wyman diagrams he has drawn (Knicks, Lakers, Spurs, Timberwolves, Thunder) were measured shape by shape; the 41 players in them are drawn exactly as he drew them, and say so. Everyone else is drawn after the drawn players he most resembles: each player sits in a space of both ends of DPM, assists per unit of on-ball creation and minutes per game, standardised over every regular since 2000-01, and his outline is the vote of his ${model.nn?.k ?? 6} nearest drawn players (their family) at the average of their fills, weighted by nearness. The row names them. Honestly stated: predicting one of his 41 shapes from the other 40 this way explains ${Math.round((model.nn?.loo.fillR2 ?? 0) * 100)}% of the variation and lands within 0.15 of his fill ${Math.round((model.nn?.loo.within ?? 0) * 100)}% of the time; a PCA over fourteen features did worse than chance. His shapes are mostly judgement, and this is the nearest a number gets. A lineup model (${R.lineups.toLocaleString()} real five-man lineups since ${seasonLabel(R.seasons[0])}) still gives each player a <b>Fit</b>, points per 100 beyond his DPM, shown in the table.</p>` +
