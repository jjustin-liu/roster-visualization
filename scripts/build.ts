...
import { predictLineup, valueOverReplacement, type PortabilityModel, SIZE_EXPONENT, type RosterShapePlate } from '../src/lib/index';
...
function buildSeason(season: number): void {
  ...
  const howBlock = (teamNote: string) =>
    ...
    `<p><b>Area is how good the player is.</b> A shape&rsquo;s area is his value &mdash; DPM above a replacement player (&minus;2.0) times his minutes per game as a share of 48, with his playoff reading applied (below) &mdash; to the power ${SIZE_EXPONENT}, because that is how his diagrams scale it (a log fit of his 24 drawn areas on value, R&sup2; 0.76): stars are drawn much bigger than role players, more than linearly, and a player is sized by his role, not by the games he missed. The box is sized for the season: 1.4 times the fullest roster of the year, so the fullest reads about 71% the way his Spurs and Knicks diagrams are about three-quarters full, and grown only if some roster still could not pack at true size. Every above-replacement player is inside it, to scale, and nothing is shrunk. Each page says what the season&rsquo;s box is worth as a team, so seasons can be compared. Players below replacement add nothing to the box, so they sit in a strip under it as hollow outlines, sized by what they cost.</p>` +
    `<p><b>Outline is how easily he fits.</b> Squares and rectangles sit flush; circles, diamonds and stars leave white around them. The outline comes from two things: a lineup model, and his three diagrams. The model reads ${R.lineups.toLocaleString()} real five-man lineups since ${seasonLabel(R.seasons[0])} and asks how much better or worse a group played than the sum of its players&rsquo; DPM; a player&rsquo;s <b>Fit</b> is the fitted effect of putting him in a typical lineup, in points per 100 beyond his DPM. The three Wyman diagrams he has drawn (Knicks, Lakers, Spurs) were measured shape by shape and used to calibrate what a clean or an awkward outline is; the 24 players in them are drawn as he drew them, and say so.</p>` +
